...
#           official ATT&CK Navigator (techniques scored by evidence count)
...
def build_navigator_layer(techniques, metadata):
    """
    Build a MITRE ATT&CK Navigator layer (v4.5) from the detected techniques so
    an analyst can drop the run straight into the official Navigator and see the
    sample's coverage highlighted on the matrix.
    ...
    """
    name = 'Noriben {}'.format(metadata.get('source_csv') or metadata.get('command_line') or '').strip()
    max_score = max([len(t.get('evidence', [])) for t in techniques], default=1) or 1

    layer_techniques = []
    for tech in techniques:
        score = len(tech.get('evidence', [])) or 1
        layer_techniques.append({
            'techniqueID': tech['id'],
            'tactic': tech.get('tactic', '').lower().replace(' ', '-'),
            'score': score,
            'enabled': True,
            'comment': '; '.join(tech.get('evidence', []))[:500],
            'metadata': [{'name': 'technique', 'value': tech.get('technique', '')},
                         {'name': 'source', 'value': 'Noriben v{}'.format(metadata.get('version', ''))}]
        })

    return {
        'name': name or 'Noriben analysis',
        'versions': {'attack': '14', 'navigator': '4.9.5', 'layer': '4.5'},
        'domain': 'enterprise-attack',
        'description': 'Behavioral ATT&CK coverage generated by Noriben from a dynamic analysis run.',
        'sorting': 3,
        'hideDisabled': True,
        ...
        'legendItems': [{'label': 'Observed by Noriben', 'color': '#ff6666'}],
...
def parse_csv(csv_file, report, timeline):
    ...
    _write_export(config.get('gen_sigma'), '.sigma.yml', 'Sigma rules',
                  ...
                  as_json=False)
    _write_export(config.get('navigator_export'), '.navigator.json', 'ATT&CK Navigator layer',
                  lambda: build_navigator_layer(attack_techniques, report_metadata))
...
def main():
    ...
    parser.add_argument('--navigator', action='store_true',
                        help='Export a MITRE ATT&CK Navigator layer (*.navigator.json)', required=False)
